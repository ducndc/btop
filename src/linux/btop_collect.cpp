...
namespace Cpu {
	...
	string get_cpuHz() 
	{
		...
		try 
		{
			...
			{
				if (hz >= 10000) 
					cpuhz = to_string((int)round(hz / 1000)); // Future proof until we reach THz speeds :)
				else 
...
#undef LOAD_SYM
		...
		{ 
			...
			for (unsigned int i = 0; i < device_count; ++i) 
			{
				...
				if constexpr(is_init) 
				{ // there doesn't seem to be a better way to do this, but this should be fine considering it's just 2 lines
					pcie_tx_thread.join();
					pcie_rx_thread.join();
				} 
...
namespace Mem {
	...
	fs::path get_zfs_stat_file(const string& device_name, size_t dataset_name_start, bool zfs_hide_datasets) 
	{
		...
		string filename;
		string name_compare;
		if (dataset_name_start != std::string::npos) 
		{ 
			// device is a dataset
		...
		if (not no_update and errors < 3) 
		{
			...
			for (const auto& iface : interfaces) 
			{
				...
				for (const string dir : {"download", "upload"}) 
				{
					...
					{
						if (net_sync and saved_stat.speed < net.at(iface).stat.at(dir == "download" ? "upload" : "download").speed) 
							continue;
